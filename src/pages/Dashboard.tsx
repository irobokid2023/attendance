...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      // Weekly attendance trend (last 8 unique session dates)
      ...
      att.forEach(a => {
        ...
        if (sid && schoolMap[sid]) {
          schoolMap[sid].total++;
          if (a.status === 'present') schoolMap[sid].present++;
        }
      });
      ...
      // Class-wise breakdown (top 10 by records)
    ...
    <DashboardLayout>
      ...
      <Card className="animate-fade-in mb-8">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <CalendarDays className="w-4 h-4" /> Upcoming Holidays
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {upcomingHolidays.map(h => (
                <div key={h.id} className="flex items-start gap-3 p-3 rounded-lg border bg-muted/30">
                  <div className="text-center min-w-[48px] rounded-md bg-primary/10 p-2">
                    <p className="text-xs font-medium text-primary">{format(parseISO(h.date), 'MMM')}</p>
                    <p className="text-lg font-bold text-primary">{format(parseISO(h.date), 'dd')}</p>
                  </div>
                  <div className="min-w-0">
                    <p className="font-medium text-foreground truncate">{h.name}</p>
                    <Badge variant="outline" className="text-xs mt-1">{(h as any).schools?.name}</Badge>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {totalToday > 0 && <Progress value={(stats.todayPresent / totalToday) * 100} className="mt-2 h-2 [&>div]:bg-success" />}
              </div>
              <div>
                <p className="text-3xl font-bold font-heading text-destructive">{stats.todayAbsent}</p>
                <p className="text-sm text-muted-foreground">Absent</p>
                {totalToday > 0 && <Progress value={(stats.todayAbsent / totalToday) * 100} className="mt-2 h-2 [&>div]:bg-destructive" />}
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        ...
        <Card className="animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">School-wise Attendance %</CardTitle>
          </CardHeader>
          <CardContent>
            {schoolData.length > 0 ? (
        ...
        <Card className="animate-fade-in" style={{ animationDelay: '0.3s' }}>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Class-wise Breakdown (Top 10)</CardTitle>
          </CardHeader>
          <CardContent>
            {classData.length > 0 ? (
